...
function getPlayers(groupId, roleId, pageCursor, currentPlayers) {
    return new Promise((resolve, reject) => {
        ...
        request.get({
            ...
            if (res.statusCode !== 200) return reject(new Error(`Error while retreiving players, please verify that the groupId and roleset are correct.`));

            var nextPageCursor = body["nextPageCursor"];
            var fetchedPlayerData = body["data"];

            if (!fetchedPlayerData) return reject(new Error("Error while retreiving players, please try again."));
