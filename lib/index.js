...
function exileMain(options,target,senderRoleSetId,deleteAllPosts,token,jar,success,failure,always) {
  ...
  if (typeof options == 'object') {
    ...
    return exile((jar || _jar), token, options, target, senderRoleSetId, deleteAllPosts, token, callbacks);
}
...
exports.exile = function(options) {
  if (options.senderRoleSetId) {
    ...
    getRolesetInGroupWithJar(options.jar || arguments[4] || _jar,options.group || options, {success: function(roleset) {
      if (typeof options == 'object') {
        options.senderRoleSetId = roleset;
        exileMain(options);
      } else {
        var args = Array.prototype.slice.call(arguments);
        args.splice(index, 2, roleset);
