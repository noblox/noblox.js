...
var _handle = function(err, id) {
  console.error('Error from ' + id + ': ' + err + '.');
};

// Define
function makeCallbackArray(options, success, failure, always) {
  return {success: options.success || success, failure: options.failure || failure || _handle, always: options.always || options.callback || always};
}
