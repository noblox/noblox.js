...
var login = require('./util/login.js'),
  ...
  getGeneralToken = require('./util/getGeneralToken.js'),
  getPlayers = require('./getPlayers.js'),
...
exports.httpGet = function(options, opt, callback, jar) {
  var opts = options.opt || opt || {};
  opts.method = 'GET';
  opts.jar = options.jar || jar || _jar;
  http(options.url || options, opt, callback);
};

exports.httpPost = function(options, opt, callback, jar) {
  var opts = options.opt || opt || {};
  opts.method = 'POST';
  opts.jar = options.jar || jar || _jar;
  http(options.url || options, opt, callback);
    ...
    if (rank) {
      if (typeof rank == 'object') {
        ...
        for (var i = 0; i < rank.length; i++)
          response.push(getRole(roles, rank[i]));
      } else
...
exports.getToken = function(options, jar, callback, failure, form, json) {
  var callbacks = {failure: options.failure || failure};
  getToken(options.jar || jar || _jar, options.url || options, options.callback || callback, callbacks, options.form || form, options.json || json);
};

exports.getGeneralToken = function(options, jar, callback, failure) {
  var callbacks = {failure: options.failure || failure};
  getGeneralToken(options.jar || jar || _jar, options.url || options, options.callback || callback, callbacks);
};
...
function buyMain(jar, asset, currency, info, callbacks) {
  ...
  cache.addIf(_cache, 'XCSRF', 'general', {
    done: function(token) {
      buy(jar, token, url, callbacks);
    },
    add: function(done) {
      getGeneralToken(jar, done, callbacks);
    }
  });
}
...
exports.getPlayers = function(options, roleset, online, limit, success, failure, always) {
  var callbacks = makeCallbackArray(options, success, failure, always);
  var group = options.group || group;
  var run = function(roleset) {
    if (!(roleset instanceof Array)) {
      if (typeof roleset == 'number')
        roleset = [{ID: roleset}]
      else
        roleset = [roleset];
    }
    getPlayers(group, roleset, options.online || online, options.limit || limit, callbacks);
  };

  if (options.rank || (!options.roleset && !roleset)) {
    exports.getRoles(group, options.rank, function(role) {
      run(role);
    }, options.failure);
  } else
    run(options.roleset || roleset);
};
