...
var _login = require('./util/login.js'),
  ...
  _getRoles = require('./util/getRoles.js'),
  _getToken = require('./util/getToken.js'),
  _message = require('./message.js');
...
function getToken(options,jar,callback,failure) {
  var callbacks = {failure: options.failure || failure};
  if (typeof options == 'object') {
    var j = options.jar;
    return _getToken((j ? j : _jar), options.url, callback, callbacks);
  } else
    return _getToken((jar ? jar : _jar), options, callback, callbacks);
}
...
function message(options,subject,body,token,jar,success,failure,always) {
  var callbacks = makeCallbackArray(options,success,failure,always);
  if (typeof options == 'object') {
    var j = options.jar;
    return _message((j ? j : _jar), options.recipient, options.subject, options.body, options.token, callbacks);
  } else
    return _message((jar ? jar : _jar), options, subject, body, token, callbacks);
}
