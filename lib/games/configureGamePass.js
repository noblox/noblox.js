    ...
    return http(httpOpt).then(function (res) {
      ...
      if (json.isValid) {
        ...
        if (res.status === 403) {
          ...
          reject(new Error(`An unexpected error occurred with status code ${res.status}.${priceComment}`))
        }
      }
  ...
  return http(httpOpt).then(function (res) {
    if (res.data.isValid) {
      ...
      if (res.status === 403) {
        ...
        throw new Error(`An unexpected error occurred with status code ${res.status}.`)
      }
    }
  })
}
