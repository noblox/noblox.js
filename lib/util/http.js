...
exports.func = function (args) {
  ...
  opt.followRedirect = function (res) {
    if (res.headers.location && (res.headers.location.startsWith('https://www.roblox.com/newlogin') || res.headers.location.startsWith('/Login/Default.aspx'))) {
      return false;
    }
    return follow;
  };
  return http(args.url, opt).then(function (res) {
    ...
    if (res.statusCode === 302 && res.headers.location && (res.headers.location.startsWith('https://www.roblox.com/newlogin') || res.headers.location.startsWith('/Login/Default.aspx')) && !args.ignoreLoginError) {
