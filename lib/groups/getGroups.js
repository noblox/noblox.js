...
function getGroups (userId) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(requests).then(async (promiseResponses) => {
      ...
      const failedResponse = (responses[0].status !== 200 || !responses[0].body) // we only check the first request because the second one errors if a primary is not set
      ...
      if (failedResponse) {
        ...
        if (body.errors && body.errors.length > 0) {
          ...
          reject(new Error(`${responses[0].status} ${errors.join(', ')}`))
        }
