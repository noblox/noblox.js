    ...
    if (res.statusCode === 302) {
      ...
      for (var i = 0; i < posts.length; i++) {
        ...
        if (pageNumber === 1 && i === 0 && id !== postId) {
          ...
          var date = getDate({time: tag.text().substring(8), timezone: 'CT'}); // Source dates are in CT, when viewed in a browser they are converted to local time: this was kind of confusing
          thread.posts.push({
            author: {
              id: userId,
              name: user.text(),
