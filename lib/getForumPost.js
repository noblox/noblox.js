...
function doPage (thread, postId, pageNumber, target, resolve, reject) {
  if (debug) {
    console.log('Do page ' + pageNumber + ' of ' + postId + ' for ' + JSON.stringify(target) + ' out of ' + thread.totalPages);
  }
  var httpOpt = {
  ...
  .then(function (res) {
    if (res.statusCode === 302) {
      ...
      for (var i = 0; i < posts.length; i++) {
        var post = posts.eq(i);
        var tag = post.find('.normalTextSmaller').eq(3);
        var id = parseInt(tag.find('a').attr('name'), 10);
        last = id;
        if (pageNumber === 1 && i === 0 && id !== postId) {
          ...
          var join = new Date(span.eq(0).text().substring(8) + ' CST');
          ...
          var date = new Date(tag.text() + ' CST'); // Source dates are in CST, when viewed in a browser they are converted to local time: this was kind of confusing
          thread.posts.push({
            author: {
              id: userId,
              name: user.text(),
      ...
      if (target && thread.posts.length === 0) {
        ...
        if (thread.totalPages < 0) {
          target.max = -1;
          return doPage(thread, postId, 1, target, resolve, reject);
        }
        if (last < target.id) {
          min = pageNumber + 1;
        } else {
        ...
        if (thread.totalPages) {
          return doPage(thread, postId, Math.floor((min + max) / 2), target, resolve, reject);
        }
      }
      if (target) {
