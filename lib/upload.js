...
// Define
function upload (jar, data, itemOptions, asset, type) {
  return function (resolve, reject) {
    var httpOpt = {
      url: '//data.roblox.com/Data/Upload.ashx?assetid=' + (asset || 0),
      options: {
    ...
    if (itemOptions) {
      httpOpt.url += '&type=' + (type || 'Model') + '&genreTypeId=1&name=' +
        itemOptions.name +
        '&description=' +
        (itemOptions.description || '') +
        '&ispublic=' +
        !itemOptions.copyLocked +
        '&allowComments=' +
...
exports.func = function (args) {
  return promise(upload(args.jar, args.data, args.itemOptions, args.asset, args.type));
};
