...
exports.request = function(jar, group, username, callback, callbacks) {
  ...
  http(admin, {jar: jar}, function(err, res, body) {
    if (err) {
    ...
    http(admin, {method: 'POST', jar: jar, form: post}, function(err, res, body) {
      if (err) {
        if (callbacks.failure)
...
exports.handle = function(jar, token, form, callbacks) {
  http('http://www.roblox.com/group/handle-join-request', {method: 'POST', jar: jar, form: form, headers: {'X-CSRF-TOKEN': token}}, function(err, res, body) {
    if (callbacks.always)
      callbacks.always();
