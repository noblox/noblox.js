...
function getPlayerInfo (userId) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(requests).then((promiseResponses) => {
      ...
      const userBody = usersResponse ? usersResponse.data : {}
      const failedResponse = promiseResponses.find(presponse => presponse.status === 'rejected') || responses.find(response => !response || (!(response instanceof Array) && (response.status !== 200 || !response.data)))
      ...
      if (userBody.isBanned) {
        ...
        const responseBodies = responses.map(res => res.data)
