    ...
    http({
      ...
      .then(function ({ statusCode, body }) {
        ...
        if (success) {
          ...
          if (error) {
            reject(new Error(`${error} | userId: ${userId}, assetId: ${assetId}`))
          } else {
            reject(new Error(`An unknown error occurred with canManage() | [${statusCode}] userId: ${userId}, assetId: ${assetId}`))
          }
        }
      })
  })
