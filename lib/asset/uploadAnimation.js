...
function upload (data, itemOptions, asset, jar, token) {
  const httpOpt = {
    url: asset ? `//www.roblox.com/ide/publish/uploadexistinganimation?assetID=${asset}&isGamesAsset=False` : '//www.roblox.com/ide/publish/uploadnewanimation?AllID=1',
    options: {
  ...
  return http(httpOpt)
    .then(function (res) {
      if (res.statusCode === 200) {
        const assetId = asset || Number(res.body)
