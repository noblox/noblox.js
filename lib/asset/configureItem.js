 ...
 * @returns {Promise<void>} # TODO: update documentation
  ...
  return http({
    ...
    .then(function (json) {
      if (!json.errors) {
        ...
        } else if (json.errors[0].code === 3) { // "Cannot release the associated asset type"
          throw new Error(`Attempting to sell a copyable asset for ${args.sellForRobux} robux. (Use a boolean for sellForRobux.)`)
        } else if (json.errors[0].code === 20) { // "Cannot set the associated asset type to remove-from-release"
          // Continue and ignore error; set sellForRobux as a boolean
          return configure(args.jar, args.token, args.id, args.name, args.description, args.enableComments, args.sellForRobux, args.genreSelection, !!args.sellForRobux)
        }
