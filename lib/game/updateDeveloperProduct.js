...
const nextFunction = (jar, token, universeId, productId, name, priceInRobux, description) => {
      ...
      return http({
        ...
        if (res.statusCode === 200) {
          ...
          if (creationStatus.length > 0 && creationStatus.text().toLowerCase().indexOf('successfully updated') > -1) {
            ...
            throw new Error(creationStatus.length > 0 ? creationStatus.text().toLowerCase() : 'Create product failed')
          }
        } else {
...
exports.func = (args) => {
  ...
  return getGeneralToken({ jar: jar }).then((xcsrf) => {
    return nextFunction(jar, xcsrf, args.universeId, args.productId, args.name, args.priceInRobux, args.description)
  })
}
